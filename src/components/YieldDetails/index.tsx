...
const YieldDetails: React.FC<YieldDetailsProps> = ({
  ...
  const perDay: number = Number((1000 * roiPerDay) / ewdPrice)
  const perWeek: number = Number((1000 * roiPerWeek) / ewdPrice)
  const perMonth: number = Number((1000 * roiPerMonth) / ewdPrice)
  const perYear: number = Number((1000 * roiPerYear) / ewdPrice)
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2 flex flex-col">
        <div className="flex flex-row flex-nowrap gap-1 font-bold">
          ...
          <div className="flex flex-row py-1 px-2 w-full">
            <div className="flex items-center justify-between uppercase">{i18n._(t`EWD per $1000`)}</div>
          </div>
        </div>
