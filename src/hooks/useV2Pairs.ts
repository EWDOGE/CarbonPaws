...
import { EWD_ADDRESS, FACTORY_ADDRESS, EWD_DISTRIBUTOR_ADDRESS, EWD_VAULT_ADDRESS } from '../constants'
...
export function useVaultTVL(): TVLInfo[] {
  ...
  const farmingPools = Object.keys(VAULTS[ChainId.EWC]).map((key) => {
    return { ...VAULTS[ChainId.EWC][key] }
  })
  ...
  const distributorBalance = useMultipleContractSingleData(pairAddresses, PAIR_INTERFACE, 'balanceOf', [
    EWD_DISTRIBUTOR_ADDRESS[ChainId.EWC],
  ])
  ...
  return useMemo(() => {
    function isKnownToken(token: TokenInfo) {
      return (
        token.id.toLowerCase() == EWD_ADDRESS[chainId].toLowerCase() ||
        token.symbol == 'WEWT' ||
        token.symbol == 'EWT' ||
        token.symbol == 'RIB' ||
        token.symbol == 'USDC' ||
    ...
    function getPrice(token: TokenInfo) {
      if (token.id.toLowerCase() == EWD_ADDRESS[chainId].toLowerCase()) {
        return ewdPrice
      }
      if (token.symbol == 'WEWT' || token.symbol == 'EWT') {
        return ewtPrice
      }
...
export function useTVL(): TVLInfo[] {
  ...
  const farmingPools = Object.keys(POOLS[ChainId.EWC]).map((key) => {
    return { ...POOLS[ChainId.EWC][key], lpToken: key }
  })
...
export function useV2PairsWithPrice(
  ...
  return useMemo(() => {
    function isKnownToken(token: Token) {
      return (
        token.address.toLowerCase() == EWD_ADDRESS[chainId].toLowerCase() ||
        token.symbol == 'WEWT' ||
        token.symbol == 'EWT' ||
        token.symbol == 'RIB' ||
        token.symbol == 'USDC' ||
    ...
    function getPrice(token: Token) {
      if (token.address.toLowerCase() == EWD_ADDRESS[chainId].toLowerCase()) {
        return ewdPrice
      }
      if (token.symbol == 'WEWT' || token.symbol == 'EWT') {
        return ewtPrice
  ...
  }, [results, chainId, ewdPrice, ewtPrice, ribPrice, tokens, totalSupply])
}
