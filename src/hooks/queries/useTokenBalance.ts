...
const useTokenBalance = (tokenAddress: string) => {
  ...
  const tokenContract = useContract(addressCheckSum || undefined, ERC20_ABI, false)
  ...
  const getBalance = async (contract: Contract | null, owner: string | null | undefined): Promise<BalanceProps> => {
    try {
      ...
      //console.log('yolo', contract, owner)
      const balance = await contract?.balanceOf(owner)
      //console.log('yolo', balance)
      const decimals = await contract?.decimals()
      //console.log('yolo', decimals)
