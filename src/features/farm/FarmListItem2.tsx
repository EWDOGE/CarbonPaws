...
import { EWD_ADDRESS } from '../../constants/tokens'
...
const FarmListItem2 = ({ farm, ...rest }) => {
  ...
  function getTvl() {
    let lpPrice = 0
    let decimals = 18
    if (farm.lpToken.toLowerCase() == EWD_ADDRESS[chainId].toLowerCase()) {
      lpPrice = ewdPrice
                ...
                <div className="flex col-span-2 space-x-4 md:col-span-1">
                  ...
                  <div className={`flex flex-col justify-center ${token1 ? 'md:flex-row' : ''}`}>
                    <div>
                      ...
                      {!token1 && token0?.symbol == 'EWD' && (
                        <div className="flex flex-col">
                          <span className="underline text-emphasis hover:text-yellow">Unstake</span>
                          <Link href="/vaults">
                            <span className="underline text-emphasis hover:text-yellow">Use Vaults</span>
                          </Link>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex items-center space-x-2">
                    {farm?.rewards?.map((reward, i) => (
                      <div key={i} className="flex items-center">
                        <Image
                          src={`https://raw.githubusercontent.com/EWDOGE/assets/master/CarbonPaws/logo/0x16e13C4cCcC031a0D7BAa34bcB39Aaf65b3C1891.png`}
                          width="50px"
                          height="50px"
