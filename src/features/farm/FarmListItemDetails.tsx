...
import { EWD_DISTRIBUTOR_ADDRESS, MINICHEF_ADDRESS } from '../../constants/addresses'
...
const FarmListItem = ({ farm }) => {
  ...
  const pending = usePendingSolar(farm)
  ...
  const [approvalState, approve] = useApproveCallback(typedDepositValue, EWD_DISTRIBUTOR_ADDRESS[chainId])
  ...
  return (
    ...
    >
      <Disclosure.Panel className="flex flex-col w-full border-t-0 rounded rounded-t-none bg-dark-800" static>
          ...
          <div className="col-span-2 text-center md:col-span-1">
            ...
            {account && (
              <div className="pr-4 mb-2 text-left cursor-pointer text-secondary">
                ...
                {farm.lpPrice && amount
                  ? ` (` + formatNumberScale(farm.lpPrice * Number(amount?.toSignificant(8)), true, 2) + `)`
                  : ``}
              </div>
            )}
            <div className="relative flex items-center w-full mb-4">
        ...
        {pending && pending.greaterThan(ZERO) && (
          <div className="px-4 pb-4">
            ...
            >
              {i18n._(t`Harvest ${formatNumber(pending.toFixed(18))} EWD`)}
            </Button>
          </div>
        )}
      </Disclosure.Panel>
