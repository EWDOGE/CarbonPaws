...
export function useSolarPositions(contract?: Contract | null) {
  ...
  return useMemo(() => {
    if (!pending || !userInfo || !userLockedUntil) {
      return []
    }
    return zip(pending, userInfo, userLockedUntil)
      .map((data, i) => ({
        id: args[i][0],
  ...
  }, [args, pending, userInfo, userLockedUntil])
}
...
export function useSolarPrice() {
  // eslint-disable-next-line react-hooks/rules-of-hooks
  return usePrice(useEWDEWTContract())
}

export function useBNBPrice() {
  // eslint-disable-next-line react-hooks/rules-of-hooks
  return usePrice(useEWTDAIContract())
}
