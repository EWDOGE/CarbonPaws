...
import { PARTY_ADDRESS } from '../../constants'
...
export default function CurrencyInputPanel({
  ...
  const { allowance, approve, leave } = useSusuParty()

  const xSushiBalanceBigInt = useTokenBalance(PARTY_ADDRESS[chainId ?? ChainId.EWC] ?? '0x')
            ...
            {!allowance || Number(allowance) === 0 ? (
              ...
              <ButtonSelect
                ...
                onClick={async () => {
                  setPendingTx(true)
                  if (maxSelected) {
                    await leave(maxDepositAmountInput, account)
                  } else {
                    await leave(formatToBalance(depositValue, decimals), account)
                  }
                  setPendingTx(false)
                }}
              >
