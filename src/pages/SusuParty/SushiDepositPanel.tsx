...
import { formatFromBalance, formatToBalance } from '../../utils/sushi'

import { SUSU_ADDRESS } from '../../constants'
import useSusuParty from '../../hooks/carbonswap/useSusuParty'
...
export default function CurrencyInputPanel({
  ...
  const { allowance, approve, enter } = useSusuParty()

  //console.log('yolo', { account, allowance, approve, enter })

  const sushiBalanceBigInt = useTokenBalance(SUSU_ADDRESS[chainId ?? ChainId.EWC] ?? '0x')
            ...
            {!allowance || Number(allowance) === 0 ? (
              ...
              <ButtonSelect
                ...
                onClick={async () => {
                  setPendingTx(true)
                  if (maxSelected) {
                    await enter(maxDepositAmountInput, account)
                  } else {
                    await enter(formatToBalance(depositValue, decimals), account)
                  }
                  setPendingTx(false)
                }}
              >
