...
const NETWORK_URL = 'https://rpc.ewc.moonbeam.network'
...
async function faucetSend(req) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    resolve({
      status: 200,
      message: `You will receive EWT in your wallet soon.`,
    })
  })
}
...
async function checkBridgeUsage(address) {
  const bridges = {
    ETH: `https://bridgeapi.anyswap.exchange/v2/swapin/history/${address}/246/1/allv2?offset=0&limit=1`,
    BSC: `https://bridgeapi.anyswap.exchange/v2/swapin/history/${address}/246/56/allv2?offset=0&limit=1`,
  }
...
export default async function handler(req, res) {
  ...
  if (verified) {
    ...
    if (usedBridge) {
      const ewtBalance = parseFloat(web3.utils.fromWei(await web3.eth.getBalance(address)))
      if (ewtBalance < 0.001) {
        ...
        res.status(200).json({
          status: 403,
          message: 'Your current EWT balance is above the minimum requirement to use the faucet.',
        })
      }
    } else {
      res.status(200).json({
