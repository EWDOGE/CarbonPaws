...
import { CurrencyAmount, Fraction, getNativeCurrency, JSBI, Percent, Token, TokenAmount, WETH } from '@uniswap/sdk'
...
import { getMigratorAddress } from '../../constants/abis/migrator'
    ...
    <>
      ...
      <RowBetween mb="1rem">
        ...
        <RowFixed>
          ...
          <CurrencyLogo size="20px" style={{ marginLeft: '8px' }} currency={getNativeCurrency(NETWORK_CHAIN_ID)} />
        </RowFixed>
      </RowBetween>
    </>
  )
...
function V1PairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
  ...
  const ethWorth: CurrencyAmount = exchangeETHBalance
    ? CurrencyAmount.native(NETWORK_CHAIN_ID, exchangeETHBalance.multiply(shareFraction).multiply(WEI_DENOM).quotient)
    : CurrencyAmount.native(NETWORK_CHAIN_ID, ZERO)

  const tokenWorth: TokenAmount = exchangeTokenBalance
    ? new TokenAmount(token, shareFraction.multiply(exchangeTokenBalance.raw).quotient)
    : new TokenAmount(token, ZERO)

  const [approval, approve] = useApproveCallback(liquidityTokenAmount, getMigratorAddress(NETWORK_CHAIN_ID))
  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        ...
        {chainId && (
          <ExternalLink href={getExplorerLink(chainId, getMigratorAddress(chainId), 'address')}>
