...
export default function Bridge() {
  ...
  const anyswapCurrencyContract = useAnyswapTokenContract(
    currency0 && currency0.chainId == ChainId.EWC && tokenToBridge.other.ContractAddress,
    true
  )
    ...
    let tokens: Currency[] = Object.keys((anyswapInfo && anyswapInfo[chainFrom.id]) || {})
      ...
      .map((r) => {
        ...
        if (r.toLowerCase() == WNATIVE[chainFrom.id].address.toLowerCase()) {
          if (chainFrom.id == ChainId.EWC) {
            return EnergyWeb.onChain(chainFrom.id)
          }
  ...
  const handleChainFrom = useCallback(
    (chain: Chain) => {
      ...
      if (changeTo.id !== ChainId.EWC && chain.id !== ChainId.EWC) {
  ...
  const handleChainTo = useCallback(
    (chain: Chain) => {
      ...
      if (changeFrom.id !== ChainId.EWC && chain.id !== ChainId.EWC) {
  ...
  const bridgeToken = async () => {
    ...
    const depositAddress = currency0.chainId == ChainId.EWC ? token.ContractAddress : token.DepositAddress

    const amountToBridge = ethers.utils.parseUnits(currencyAmount, token.Decimals)
    setPendingTx(true)

    try {
      if (currency0.chainId == ChainId.EWC) {
  ...
  const anyswapChains = [ChainId.EWC, ChainId.BSC, ChainId.MAINNET]
  ...
  return (
    <>
      ...
      <Head>
        <title>{i18n._(t`Bridge`)} | CarbonPaws</title>
            ...
            <div className="p-4 text-center">
              <div className="items-center justify-between space-x-3">
                <Typography component="h3" variant="base">
                  {i18n._(t`Bridge tokens to and from the EnergyWeb Network`)}
                </Typography>
              </div>
            </div>
